...
const MentorshipChat = () => {
  ...
  const mentors = [
    {
      ...
      lastMessage: 'Great progress on your coding skills! Keep practicing those algorithms.',
    ...
    },
    {
      ...
      avatar: 'https://images.pexels.com/photos/697509/pexels-photo-697509.jpeg?auto=compress&cs=tinysrgb&w=50&h=50&fit=crop&crop=face',
      lastMessage: 'I\'ve reviewed your portfolio. Let\'s schedule a call to discuss PM roles.',
    ...
    },
    {
      ...
      role: 'Data Scientist',
      avatar: 'https://images.pexels.com/photos/697509/pexels-photo-697509.jpeg?auto=compress&cs=tinysrgb&w=50&h=50&fit=crop&crop=face',
      lastMessage: 'Shared some resources for your upcoming data science interviews.',
  ...
  const messages = [
    ...
    },
    {
      ...
      content: 'Hi Sarah! I\'ve been working on dynamic programming problems. Still finding some of them challenging.',
  ...
  const selectedMentor = mentors.find(mentor => mentor.id === selectedChat);
          ...
          <div className="flex-1 overflow-y-auto">
            {mentors.map((mentor) => (
              <motion.div
                ...
                className={`p-4 cursor-pointer border-b border-slate-800/50 dark:border-slate-800/50 border-gray-200/50 transition-all duration-200 ₹{selectedChat === mentor.id ? 'bg-blue-500/10 border-l-4 border-l-blue-500' : ''
                  }`}
              >
                <div className="flex items-start space-x-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <h4 className="text-sm font-medium text-white text-gray-900 truncate dark:text-white">{mentor.name}</h4>
                    ...
                    <p className="mb-1 text-xs text-gray-600 text-slate-400 dark:text-slate-400">{mentor.role} at {mentor.company}</p>

                    <p className="mb-1 text-xs text-gray-700 truncate text-slate-300 dark:text-slate-300">{mentor.lastMessage}</p>

                    <div className="flex items-center justify-between">
                      <span className="text-xs text-gray-500 text-slate-400 dark:text-slate-400">{mentor.lastMessageTime}</span>
                      <div className="flex items-center space-x-1">
                        <Star className="w-3 h-3 text-yellow-400 fill-current" />
                        <span className="text-xs text-gray-500 text-slate-400 dark:text-slate-400">{mentor.rating}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          {selectedMentor ? (
              ...
              <div className="p-6 border-b bg-slate-900/50 dark:bg-slate-900/50 bg-white/80 backdrop-blur-xl border-slate-700/50 dark:border-slate-700/50 border-gray-200/50">
                <div className="flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    ...
                    <div>
                      <h3 className="font-semibold text-white text-gray-900 dark:text-white">{selectedMentor.name}</h3>
                      <p className="text-sm text-gray-600 text-slate-400 dark:text-slate-400">{selectedMentor.role} at {selectedMentor.company}</p>
                      <div className="flex items-center mt-1 space-x-4">
                        <div className="flex items-center space-x-1">
                          <Star className="w-3 h-3 text-yellow-400 fill-current" />
                          <span className="text-xs text-gray-500 text-slate-400 dark:text-slate-400">{selectedMentor.rating} rating</span>
                        </div>
                        <span className="text-xs text-gray-500 text-slate-400 dark:text-slate-400">{selectedMentor.sessions} sessions</span>
                      </div>
                    </div>
                  </div>
              ...
              <div className="flex-1 p-6 space-y-4 overflow-y-auto">
                {messages.map((msg) => (
                  <motion.div
                    ...
                    className={`flex ₹{msg.isFromMentor ? 'justify-start' : 'justify-end'}`}
                  >
                    <div className={`max-w-xs lg:max-w-md ₹{msg.isFromMentor ? 'order-2' : 'order-1'}`}>
                      {msg.isFromMentor && (
                        <div className="flex items-center mb-1 space-x-2">
                          ...
                          />
                          <span className="text-xs text-gray-600 text-slate-400 dark:text-slate-400">{selectedMentor.name}</span>
                          <span className="text-xs text-gray-500 text-slate-500 dark:text-slate-500">{formatTime(msg.timestamp)}</span>
                        </div>
                      )}

                      <div className={`p-3 rounded-2xl ₹{msg.isFromMentor
            ...
            <div className="flex items-center justify-center flex-1">
              <div className="text-center">
                ...
                <h3 className="mb-2 text-xl font-semibold text-white text-gray-900 dark:text-white">Select a Mentor</h3>
                <p className="text-gray-600 text-slate-400 dark:text-slate-400">Choose a mentor from the sidebar to start chatting</p>
              </div>
            </div>
          )}
        </div>
        ...
        {selectedMentor && (
          <div className="p-6 border-l w-80 bg-slate-900/50 dark:bg-slate-900/50 bg-white/80 backdrop-blur-xl border-slate-700/50 dark:border-slate-700/50 border-gray-200/50">
            <div className="mb-6 text-center">
              ...
              />
              <h3 className="text-lg font-semibold text-white text-gray-900 dark:text-white">{selectedMentor.name}</h3>
              <p className="text-gray-600 text-slate-400 dark:text-slate-400">{selectedMentor.role}</p>
              <p className="text-sm text-gray-600 text-slate-400 dark:text-slate-400">{selectedMentor.company}</p>
            </div>

            <div className="space-y-4">
              <div>
                <h4 className="mb-2 font-medium text-white text-gray-900 dark:text-white">Expertise</h4>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="text-center">
                  <p className="text-2xl font-bold text-white text-gray-900 dark:text-white">{selectedMentor.rating}</p>
                  <p className="text-sm text-gray-600 text-slate-400 dark:text-slate-400">Rating</p>
                </div>
                <div className="text-center">
                  <p className="text-2xl font-bold text-white text-gray-900 dark:text-white">{selectedMentor.sessions}</p>
